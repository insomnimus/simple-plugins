...
impl Oversampler1 {
	...
	/// Get the latency ofthis [`Oversampler1`].
...
/// A linear-phase oversampler you can change the ratio of.
///
/// This wraps an [`Oversampler1`] per oversampling factor requested in [`AdOversampler1::new`].
/// That is, for a maximum factor of 3, this will wrap 3 [Oversampler1s][Oversampler1].
/// Note that the memory use grows exponentially as more oversamplers are used.
pub struct AdOversampler1 {
	orig_sample_rate: usize,
	block_size: usize,
	factor: u8,
	oversamplers: Box<[Oversampler1]>,
}

impl AdOversampler1 {
	/// Create a new [`AdOversampler1`].
	///
	/// # Parameters
	///
	/// - `block_size`: How many samples should be buffered. Lower values decrease latency but increase CPU load, higher values do the opposite. A good value is whatever the host provides you with, or half that. Panics if `0`.
	...
	pub fn new(
		...
		Self {
			...
			oversamplers: (0..max_factor)
				.map(|f| Oversampler1::new(block_size, orig_sample_rate, u8::pow(2, f as u32 + 1)))
				.collect(),
		}
	}
	...
	/// Get the latency of this [`AdOversampler1`].
	///
	...
	/// Reset state associated with this [`AdOversampler1`].
	pub fn reset(&mut self) {
		if self.factor > 0 {
	...
	///
	/// Panics if `factor` is greater than `max_factor` provided in [`AdOversampler1::new`].
