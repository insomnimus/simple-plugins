	...
	impl Lanzcos3Stage {
		...
		pub fn new(maximum_block_size: usize, stage_number: usize) -> Self {
			...
			Self {
				...
				upsampling_rb: vec![
					0.0;
					LANCZOS3_UPSAMPLING_KERNEL.len() + additional_delay_required
				],
		...
		pub fn upsample_from(&mut self, block: &[f32]) {
			...
			for output_sample_idx in 0..output_length {
				...
				self.upsampling_rb[self.upsampling_write_pos] =
...
impl AdOversampler2 {
	...
	///
	/// # Panics
	/// Panics if `sample`'s length is longer than the maximum block size specified in [AdOversampler2::new].
